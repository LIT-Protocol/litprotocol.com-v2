...
const LandingHero = () => {
  ...
  useEffect(() => {
    ...
    const itemElements = Array.from(scrollContainer.children);
    const itemHeight = itemElements[0].offsetHeight;
    const totalItems = heroList.length;
    ...
    const itemTotalHeight = itemHeight + 45; // Height + margin
    const viewportHeight = scrollContainer.parentElement.offsetHeight;
    
    let animationFrame;
    let startTime;
    
    // Duration settings (in ms)
    const pauseDuration = 2000; // Time to pause on each item
    const transitionDuration = 400; // Time to move between items
    const fullCycleDuration = (pauseDuration + transitionDuration) * totalItems;
    
    // Calculate fade zone (percentage of viewportHeight)
    const fadeZoneSize = 0.4; // 40% of the viewport at top and bottom will be fade zones
    
    // Function to update opacity based on position
    const updateItemOpacity = (currentPos, transitionProgress, currentItemIndex) => {
      itemElements.forEach((item, index) => {
        const itemPos = index * itemTotalHeight;
        ...
        if (index === (currentItemIndex + 1) % totalItems && transitionProgress > 0) {
        ...
        const isNearby = Math.abs(index - currentItemIndex) <= 2 || 
                         (currentItemIndex === totalItems - 1 && index === 0);
                         
        if (isNearby) {
          // Fade out at top of viewport
          if (normalizedPos < 0 && normalizedPos > -fadeZoneSize) {
            opacity = normalizedPos / -fadeZoneSize;
          } 
          // Fade out at bottom of viewport
          else if (normalizedPos > 0 && normalizedPos < fadeZoneSize) {
            opacity = 1 - (normalizedPos / fadeZoneSize);
          }
        }
    ...
    const animateScroll = (timestamp) => {
      ...
      let elapsed = (timestamp - startTime) % fullCycleDuration;
      
      // Determine which item we should be showing and how far through transition
      const cyclePosition = elapsed / (pauseDuration + transitionDuration);
      const currentItemIndex = Math.floor(cyclePosition);
      
      // Calculate how far into the current cycle we are (0 to 1)
      // 0 to pauseDuration: we're in the pause state
      // pauseDuration to (pauseDuration + transitionDuration): we're in the transition state
      const cycleProgress = (elapsed % (pauseDuration + transitionDuration)) / (pauseDuration + transitionDuration);
      
      // Calculate transition progress only during transition phase
      // Will be 0 during pause, then 0 to 1 during transition
      const transitionProgress = Math.max(0, Math.min(1, 
        (cycleProgress * (pauseDuration + transitionDuration) - pauseDuration) / transitionDuration
      ));
      
      // Position without bounce (base position during pause)
      let basePosition = currentItemIndex * itemTotalHeight;
      
      // Apply transition with bounce effect
      if (transitionProgress > 0) {
        // Enhanced bounce effect - make it more subtle with a single bounce
        const easeOutBack = (t) => {
          const c1 = .15; // Reduced bounce for subtlety
          const c3 = c1 + 1;
          return 1 + c3 * Math.pow(t - 1, 3) + c1 * Math.pow(t - 1, 2);
        };
        
        // Simple easing function for a single bounce
        const singleBounceOut = (t) => {
        ...
        const easeValue = 0.8 * singleBounceOut(transitionProgress) + 
                        0.2 * easeOutBack(transitionProgress);
        
        // Calculate position with enhanced bounce
        const additionalOffset = easeValue * itemTotalHeight;
      ...
      if (currentItemIndex === totalItems - 1 && transitionProgress > 0) {
        ...
        itemElements[0].style.transform = `translateY(${itemTotalHeight * totalItems}px)`;
      } else {
        ...
        <div className="flex justify-center items-center">
          <div className="flex flex-col items-center text-center p-4 md:p-0">
            <div className="text-center mt-[7.8rem] mb-[1.25rem]">
              <Text className="!text-white md:!text-[5.875rem]/[6.46rem]">
                Unlock Autonomy for{' '}
              </Text>
